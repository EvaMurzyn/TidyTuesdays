...
mission_numbers <- astronauts %>% 
  ...
  summarise(max_mission_length = max(hours_mission),
            mean_mission_length = mean(hours_mission),
...
mission_numbers %>%
  filter(ascend_shuttle_type != "NA") %>%
  ggplot(aes(year, mean_mission_length, color = ascend_shuttle_type, size = astronauts_on_board)) +
  geom_point(alpha = 0.5) +
  theme_ft_rc() +
  labs(title = "Space launches across the years", 
       subtitle = "Astronauts ferried and vehicle type", 
       x = "Year", y = "Average mission length") + 
  theme(plot.title = element_text(family = myFont, face = "bold", hjust = 0))

ggsave("ascent_vehicles_time.png", plot = last_plot(), device = "png", path = NULL, scale = 1, width = 10, height = 9, units = "in") 
